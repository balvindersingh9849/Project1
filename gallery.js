...
// the click event handler for the image list
$("#image_list a").click(function(event) {
	event.preventDefault();
	// geting the image to be displayed
	var displayImg = $(this).attr("href");
	
	// animate display image
	$("#image").animate({opacity: 0, marginLeft: "-=205"}, 1000, function() {
	// reverseing the animation and set the URL for display image
		$(this).attr("src", displayImg).animate({opacity: 1, marginLeft: "+=205"}, 1000);
	});
});// end click	
